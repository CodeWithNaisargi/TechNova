...
interface StudentInsights {
    ...
    recommendations: {
        courses: Array<{ id: string; title: string; reason: string }>;
 ...
 */
function calculateEngagement(enrolled: number, completed: number): InsightScore {
    if (enrolled === 0) {
 ...
 */
async function calculateSkillGrowth(userId: string): Promise<{
    score: number;
    level: string;
    skillsAcquired: string[];
    totalSkills: number;
 ...
 */
async function calculateCareerAlignment(userId: string): Promise<{
    score: number;
    matchedSkills: string[];
    missingSkills: string[];
    alignmentPercentage: number;
    ...
    if (!careerPath && user.careerFocusId) {
        ...
        if (focusId.includes('agri')) domainMatch = 'AGRICULTURE';
        else if (focusId.includes('health')) domainMatch = 'HEALTHCARE';
        else if (focusId.includes('urban')) domainMatch = 'URBAN';
        else if (focusId.includes('tech') || focusId.includes('data') || focusId.includes('engineering') || focusId.includes('science')) domainMatch = 'TECH';
    ...
    const matchedSkills = careerSkills.filter(cs =>
        userSkillNames.includes(cs.toLowerCase())
    );
 ...
 */
async function calculateConsistency(userId: string): Promise<InsightScore> {
...
async function generateRecommendations(
    ...
    const recommendedCourses = await prisma.course.findMany({
        where: {
            isPublished: true,
            ...(educationLevel && { targetEducationLevel: educationLevel as any }),
            OR: [
 ...
 */
export async function generateInsights(userId: string): Promise<StudentInsights | null> {
    ...
    const careerReadinessPercentage = Math.round(
    ...
    if (careerReadinessPercentage >= 80) careerStatus = 'CAREER_READY';
    else if (careerReadinessPercentage >= 60) careerStatus = 'ON_TRACK';
    else if (careerReadinessPercentage >= 40) careerStatus = 'DEVELOPING';
    else careerStatus = 'EARLY_STAGE';
