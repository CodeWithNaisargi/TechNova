...
export const toggleAssignmentCompletion = async (req: Request, res: Response) => {
    try {
        ...
        const existingProgress = await prisma.assignmentProgress.findFirst({
            where: {
                assignmentId,
                userId: studentId,
            },
        ...
        if (existingProgress) {
            ...
            progress = await prisma.assignmentProgress.update({
...
export const getAssignmentProgress = async (req: Request, res: Response) => {
    try {
        ...
        const progressData = enrollments.map((enrollment) => {
            ...
            const completedAssignments = enrollment.course.assignments.filter(
                (assignment) =>
                    assignment.progress.length > 0 && assignment.progress[0].status === 'COMPLETED'
            ).length;
