...
exports.getStudentVector = getStudentVector;
exports.getCourseVector = getCourseVector;
exports.getRecommendations = getRecommendations;
exports.getNextFocusSkill = getNextFocusSkill;
exports.inferSkillsFromCourseCompletion = inferSkillsFromCourseCompletion;
...
async function getStudentVector(userId) {
    ...
    const skillScores = CORE_SKILLS.map(skillName => {
        const userSkill = user.userSkills.find((us) => us.skill.name.toLowerCase().includes(skillName.toLowerCase()));
 ...
 */
async function getCourseVector(courseId) {
    ...
    const skillScores = CORE_SKILLS.map(skillName => {
        ...
        const hasSkill = course.courseSkills.some(cs => cs.skill.name.toLowerCase().includes(skillName.toLowerCase()));
        // Also check tags
        const inTags = course.tags.some(tag => tag.toLowerCase().includes(skillName.toLowerCase()));
...
async function getRecommendations(userId, limit = 10) {
    ...
    const recommendations = await Promise.all(courses
 ...
 */
async function getNextFocusSkill(userId) {
    const user = await prisma_1.default.user.findUnique({
        where: { id: userId },
        include: {
 ...
 */
async function inferSkillsFromCourseCompletion(userId, courseId) {
    const course = await prisma_1.default.course.findUnique({
        where: { id: courseId },
    ...
    for (const skill of skillsToAdd) {
        await prisma_1.default.userSkill.upsert({
            where: {
